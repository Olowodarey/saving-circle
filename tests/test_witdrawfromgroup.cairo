...
//     start_cheat_caller_address, start_cheat_block_timestamp, stop_cheat_caller_address,
...
//     // Mint tokens to user and approve contract
...
//         contract_address, token_address, owner, user1, contribution_amount, token_amount, TimeUnit::Days, 1
//     );
...
//     assert(total_locked == contribution_amount * 3, 'Total locked incorrect');
...
//     // Note: In a real scenario, the group state would be set to Completed after all payouts

//     // Test withdrawal after payout (using existing withdraw_locked function)
//     let initial_balance = token_dispatcher.balance_of(user1);
    
//     start_cheat_caller_address(contract_address, user1);
//     // This should work if user has completed their contributions
...
//     let group_id = setup_user_and_group(
//         contract_address, token_address, owner, user, contribution_amount, token_amount, TimeUnit::Days, 1
//     );
...
//     assert(deadline == initial_time + 79200, 'Daily deadline 22h');

//     // Check time until deadline
//     let time_until = dispatcher.get_time_until_deadline(group_id, user);
//     assert(time_until == 79200, 'Time until deadline wrong');
...
//     let group_id = setup_user_and_group(
//         contract_address, token_address, owner, user, contribution_amount, token_amount, TimeUnit::Weeks, 1
//     );
...
//     // Check deadline is set to 6 days from now (6 * 86400 = 518400 seconds)
//     let deadline = dispatcher.get_contribution_deadline(group_id, user);
//     assert(deadline == initial_time + 518400, 'Weekly deadline 6d');
...
//     let group_id = setup_user_and_group(
//         contract_address, token_address, owner, user, contribution_amount, token_amount, TimeUnit::Months, 1
//     );
...
//     // Check deadline is set to 28 days from now (28 * 86400 = 2419200 seconds)
//     let deadline = dispatcher.get_contribution_deadline(group_id, user);
//     assert(deadline == initial_time + 2419200, 'Monthly deadline 28d');
...
//     // Move time forward past the deadline (22 hours + 1 hour = 23 hours)
//     let late_time = initial_time + 82800; // 23 hours
...
//     let expected_total_payment = contribution_amount + (contribution_amount / 100) + expected_penalty; // contribution + 1% insurance + 5% penalty

//     assert(balance_before - balance_after == expected_total_payment, 'Penalty not applied correctly');
...
//     assert(second_deadline == second_time + 79200, 'Second deadline incorrect'); // 22 hours from second contribution
